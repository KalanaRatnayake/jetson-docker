## jetson-ros-pytorch:humble-core-r35.2.1

### Pull or Build

Pull the docker container
```bash
docker pull ghcr.io/kalanaratnayake/jetson-ros-pytorch:humble-core-r35.2.1
```

Build the docker container
```bash
docker buildx build --load --platform linux/arm64 -f ros/humble/r3630.core_pytorch.Dockerfile -t jetson-ros-pytorch:humble-core-r35.2.1 .
```
...
```bash
docker run --rm -it --runtime nvidia --network host --gpus all -e DISPLAY ghcr.io/kalanaratnayake/jetson-ros-pytorch:humble-core-r35.2.1 bash
```
